...
public:
	...
	void updateOBB(int fixAxis = -1); // wll
	...
	int getSuppPlaneNum() { return m_suppPlaneBuilder->getSuppPlaneNum(); };
	SuppPlane::ModelInfo getSuppModels(int pi);

	SuppPlaneBuilder* getSuppPlaneBuilder() { return m_suppPlaneBuilder; }; // wll

	int getSuppChildrenNum() { return m_suppChindrenList.size(); };
...
private:
	...
	// supported objects
	//std::vector<CModel*> m_SuppChildren;
	//std::vector<QString> m_SuppChildrenName;
	//std::vector<MathLib::Vector3> m_SuppChildrenPos;
};
